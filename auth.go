...
// claimFromAccessToken extracts a named claim from a Bitwarden JWT access
// token. Returns "" if the token is not a JWT, is malformed, or lacks the
// claim. Used for extracting email (crypto.go email fallback) and
// organization (SM client) from JWTs.
//
...
// validated it. We only read claims, which are non-security-critical
// for our use (we'll re-validate by deriving a key and checking MAC).
func claimFromAccessToken(token, claimName string) string {
...
func emailFromAccessToken(token string) string {
	return claimFromAccessToken(token, "email")
}
